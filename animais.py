...
class Animais:
	...
	def cadastrar(self) -> None:
		self._validar_campos()
		print(f"Animal {self.nome} cadastrado com sucesso!")
	...
	def editar(
		...
		self._validar_campos()
		print(f"Dados do animal {self.nome} atualizados!")

	def remover(self, lista_animais: list[Animais] | None = None) -> None:
		"""Remove o animal de uma lista (se fornecida).

		Observação: como não há banco/lista global neste projeto, a remoção real
		depende de uma coleção externa.
		"""

		if lista_animais is None:
			print(
				f"Remoção solicitada para {self.nome}, mas nenhuma lista foi informada. "
				"Passe a lista para remover o objeto."
			)
			return

		try:
			lista_animais.remove(self)
			print(f"Animal {self.nome} removido com sucesso!")
		except ValueError:
			print(f"Animal {self.nome} não encontrado na lista.")
	...
	def adotar(self) -> None:
		...
		self.status = "adotado"
		print(f"Animal {self.nome} foi adotado!")
